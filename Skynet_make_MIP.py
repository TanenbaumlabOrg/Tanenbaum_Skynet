...
def compute_projection(z_image_storage, median_maximum):
...
def process_images(y_pixel_number, x_pixel_number, z_positions, images_z_slices, sub_z, deprecated_name, problem_channel, single_indicator, median_maximum, input_path):  
    z_image_storage = np.ndarray(shape=(y_pixel_number, x_pixel_number, z_positions), dtype=np.uint16)
    problem_channel_str = 'W0{:03}'.format(problem_channel)  # Replace f-string with format method

    for current_z_index in range(z_positions):
        if problem_channel_str in deprecated_name:
            if single_indicator == 'Multiple':
    ...
    projection = compute_projection(z_image_storage, median_maximum)
    return projection

def make_MIP(input_path, output_path, current_base_file, png_names, problem_channel, sub_z, single_indicator, median_maximum):
    ...
    if 'W0{:03}'.format(problem_channel) in deprecated_name:
        if single_indicator == 'Multiple':
    ...
    projection = process_images(y_pixel_number, x_pixel_number, z_positions, images_z_slices, sub_z, deprecated_name, problem_channel, single_indicator, median_maximum, input_path)
...
with alive_bar(number_groups) as bar:
    with ThreadPoolExecutor(int(number_worker) + 4 if number_worker != 'MusclePower' else None) as executor:
        ...
        Futures = [executor.submit(make_MIP, input_path, output_path, current_base_file, png_names, problem_channel, sub_z, single_indicator, median_maximum) for current_base_file in unique_groups]
