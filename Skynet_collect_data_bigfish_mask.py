...
# Welcome Message
print('')
print('\033[1;34;40mYou accessed subroutine "15": Make quantification using big-fish spot counts and cell mask images . \033[1;37;40m \n')

# Location of mask images to process
print('')
input_path = input('\033[2;37;40mPlease provide the location of the mask files from which to assemble the data: \033[1;37;40m \n')
...
# Which mask files are used for counting
print('')
mask_object = input('\033[2;37;40mPlease the name of the masks in which spots should be counted (exact name!) \033[1;37;40m \n')
...
def make_and_save_quantification(input_path, output_path, current_index, cell_mask_names, spot_csv_names, decomposed_spot_csv_names):
    ...
    quantification_storage = np.zeros((len(unique_labels),2),dtype = np.float64)
    ...
    for current_cell in range(len(unique_labels)):
        ...
        quantification_storage[current_cell,1] = current_decomposed_spots

    int_quantification_storage = quantification_storage.astype(int)
    column_names = ['spots','decomposed_spots']
    pandas_quantification_storage = pd.DataFrame(int_quantification_storage,columns = column_names)
    prefix = current_cell_mask_name[0:len(current_cell_mask_name)-4]
    data_name = ''.join([prefix,'_spot_quantification_B.csv'])
...
cell_mask_names = [x for x in mask_names if re.search(''.join(['_',mask_object]), x)];
...
with alive_bar(number_files) as bar:
    with ThreadPoolExecutor(int(number_worker) + 4 if number_worker != 'MusclePower' else None) as executor:
        ...
        Futures = [executor.submit(make_and_save_quantification, input_path, output_path, current_index, cell_mask_names, spot_csv_names, decomposed_spot_csv_names) for current_index in range(number_files)]
