...
# Welcome Message
print('')
print('\033[1;34;40mYou accessed subroutine "12": Do cellpose segmentation of nuclei. \033[1;37;40m \n')

# Inputs
# Location where input folders are located
print('')
input_path = input('\033[2;37;40mPlease provide the location of images from which you want to use for nucleus segmentation: \033[1;37;40m \n')
# Location where output is saved
print('')
output_path = input('\033[2;37;40mPlease provide the location of where nucleus segmentation should saved to: \033[1;37;40m \n')
# Nucleus channel
print('')
...
def prepare_image(input_path,current_nucleus_file):
    ...
    segmentation_image[:,:,0] = current_nucleus_image
    segmentation_image[:,:,1] = current_nucleus_image
    
    return segmentation_image, prefix
    
def save_nucleus_mask(output_path, mask, prefix):
    
    png_masks = im.fromarray(mask)
    mask_save_name = ''.join([output_path,'/',prefix,'_nucleus.png'])
    png_masks.save(mask_save_name)
    
def run_segmentation(input_path, output_path, current_nucleus_file, diameter, channels, model):
    
    segmentation_image, prefix = prepare_image(input_path,current_nucleus_file)
    mask = run_cellpose(segmentation_image, diameter, channels, model)
    save_nucleus_mask(output_path, mask, prefix)
    # end of functions
...
with alive_bar(len(nucleus_images)) as bar:   
    for current_nucleus_file in nucleus_images:
        run_segmentation(input_path, output_path, current_nucleus_file, diameter_center, channels, model)
   
        bar()
            
            
# Say goodbye
print('\033[1;34;40mSubroutine "12" completed. \033[1;37;40m \n')
