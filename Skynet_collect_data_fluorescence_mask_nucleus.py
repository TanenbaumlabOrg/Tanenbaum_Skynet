...
# Welcome Message
print('')
print('\033[1;34;40mYou accessed subroutine "18": Quantify fluoresence intensity in nucleus mask. \033[1;37;40m \n')
...
def make_and_save_quantification(mask_input_path, image_input_path, output_path, current_index, cell_mask_names, image_names):
    ...
    #current_nucleus_mask_name = nucleus_mask_names[current_index]
    current_prefix = current_nucleus_mask_name[0:len(current_nucleus_mask_name)-29]
    ...
    #current_nucleus_mask = cv2.imread('/'.join([mask_input_path,current_nucleus_mask_name]), cv2.IMREAD_UNCHANGED)
    y_pixel, x_pixel = np.shape(current_nucleus_mask)
    ...
    unique_labels = np.unique(current_nucleus_mask)
    # delete 0 label
    ...
    prefix = current_nucleus_mask_name[0:len(current_nucleus_mask_name)-9]
...
number_files = len(nucleus_mask_names)
...
with alive_bar(number_files) as bar:
    with ThreadPoolExecutor(int(number_worker) + 4 if number_worker != 'MusclePower' else None) as executor:
        ...
        Futures = [executor.submit(make_and_save_quantification, mask_input_path, image_input_path, output_path, current_index, nucleus_mask_names, image_names) for current_index in range(number_files)]
...
# Say goodbye
print('\033[1;34;40mSubroutine "18" completed. \033[1;37;40m \n')
